...
function publicRoom(room) {
  return { id: room.id, phase: room.phase, players: publicPlayers(room) };
}
    ...
    switch (msg.type) {
      case 'create': {
        ...
        const room = {
          ...
          phase: 'lobby', // lobby -> draft -> tournament -> over
          draftDone: new Set(),
          teams: new Map(),
          bracket: [],
          champion: null,
      ...
      case 'join': {
        ...
        if (room.phase !== 'lobby') { send(ws, { type: 'error', message: 'Partida já iniciada' }); return; }
      ...
      case 'matchResult': {
        const room = rooms.get(ws.roomCode);
        if (!room || room.phase !== 'tournament') return;
        const m = room.bracket.find(x => x.id === msg.matchId);
        if (!m || m.winnerSide) return; // já resolvida — ignora duplicata/corrida
        const claimsHome = m.home.kind === 'human' && m.home.ownerId === ws.id;
        const claimsAway = m.away.kind === 'human' && m.away.ownerId === ws.id;
        if (!claimsHome && !claimsAway) return; // não é dono dessa partida
