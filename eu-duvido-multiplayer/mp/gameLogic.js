...
function makeRoom(code, numDuplas, duration, totalRounds){
  return {
    ...
    duplas: [0,1,2].map(()=>({ name:'', players:[{name:'',connected:false},{name:'',connected:false}], score:0, apostadorTurns:0, challengerTurns:0 })),
    createdAt: Date.now(),
    pool:null, usedIds:new Set(),
    roundNum:0,
    activeIdx:0, opponentIdx:1, refereeIdx:-1,
    apostadorIdx:0, respondenteIdx:1, challengerIdx:0,
    challenge:null,
...
function startGame(room, seat){
  ...
  for(let i=0;i<3;i++){ room.duplas[i].score = 0; room.duplas[i].apostadorTurns=0; room.duplas[i].challengerTurns=0; }
  room.roundNum = 0;
  nextRound(room);
  return {ok:true};
}
...
function nextRound(room){
  ...
  const opponentDupla = room.duplas[room.opponentIdx];
  const apostadorIdx = activeDupla.apostadorTurns % 2;
  const challengerIdx = opponentDupla.challengerTurns % 2;
  activeDupla.apostadorTurns += 1;
  opponentDupla.challengerTurns += 1;
  room.apostadorIdx = apostadorIdx;
  room.respondenteIdx = 1-apostadorIdx;
  room.challengerIdx = challengerIdx;
  ...
  room.revealQueue = [
    {duplaIdx: room.activeIdx, playerIdx: apostadorIdx},
    {duplaIdx: room.opponentIdx, playerIdx: challengerIdx}
  ];
  room.revealStep = 0;
...
function allowedActorIdx(room, td){
  return td===room.activeIdx ? room.apostadorIdx : room.challengerIdx;
}
function raiseBid(room, seat, number){
  ...
  if(seat.duplaIdx!==td || seat.playerIdx!==allowedActorIdx(room,td)) return {ok:false, error:'Nao e a sua vez.'};
  const n = parseInt(number,10);
...
function buildClientView(room, seat){
  ...
  if(room.screen==='bidding'){
    const td = turnDuplaIdx(room);
    const actorIdx = allowedActorIdx(room, td);
    base.bidCurrent = room.bidCurrent;
    base.bidLog = room.bidLog[room.bidLog.length-1];
    base.turnName = duplaName(room, td);
    base.turnPlayerName = playerName(room, td, actorIdx);
    base.canAct = (seat.duplaIdx===td && seat.playerIdx===actorIdx);
    return base;
  }
